...
def ambient_pressure_inhg(
    ...
    # Preserve the 4.30-ft dyno AGL placement while making
    # altitude_ft refer to the actual ASL evaluation altitude.
    fdm.set_property_value(
...
print(
    "MOONEY AF1B PRODUCTION-CANDIDATE "
...
for temp_bias_f, rows in all_rows.items():
    ...
    print(
        "compact capability parity: "
        f"{'PASS' if capability_parity_ok else 'FAIL'}"
    )

    if not (
        transition_ok
        and tracking_ok
        and capability_parity_ok
    ):
        overall_pass = False
...
if overall_pass:
    print(
        "AF1B PRODUCTION-CANDIDATE NON-ISA "
