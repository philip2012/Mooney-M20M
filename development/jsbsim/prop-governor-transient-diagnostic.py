...
INITIAL_THROTTLE = float(
    os.environ.get(
        "AF1B_INITIAL_THROTTLE",
        "0.55",
    )
)

PRESTEP_HOLD_SEC = 8.0
...
def run_dynamic_case(
    ...
    initial_pilot_throttle = INITIAL_THROTTLE
    ...
    # Fully settle the pre-step propeller operating point.
    run_for(
        fdm,
        PRESTEP_HOLD_SEC,
    )
    ...
    for step in range(steps):
        ...
        if omega > 0.01:
            ...
            prop_load_torque_ftlb = (
                prop_power_ftlbps
                / omega
            )
        else:
            engine_torque_ftlb = 0.0
            prop_load_torque_ftlb = 0.0
        ...
        samples.append(
            {
                ...
                "rpm_dot": actual_rpm_dot,
                "rpm_dot_pred": predicted_rpm_dot,
                "hp": hp,
                "blade": blade,
    ...
    return {
        ...
        "prestep_throttle": initial_pilot_throttle,
        "prestep_rpm": prestep_rpm,
        "prestep_blade": prestep_blade,
        "prestep_map": prestep_map,
        "prestep_hp": prestep_hp,
        "prestep_prop_hp": prestep_prop_hp,
        "map": avg_map,
        "rpm": avg_rpm,
        "hp": avg_hp,
        "blade": avg_blade,
...
print(
    "MOONEY M20M PROP GOVERNOR TRANSIENT DIAGNOSTIC"
)
print(
    "========================================"
)
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    for case in TEST_CASES:
        ...
        print(
            f"              "
            f"PRE-STEP: "
            f"thr={result['prestep_throttle']:.3f}  "
            f"RPM={result['prestep_rpm']:.1f}  "
            f"blade={result['prestep_blade']:.2f}deg  "
            f"MAP={result['prestep_map']:.2f}\"  "
            f"HP={result['prestep_hp']:.1f}  "
            f"propHP={result['prestep_prop_hp']:.1f}"
        )
        ...
        print()
        print(
            " GOVERNOR TRACE -- first 3.0 seconds"
        )
        print(
            " t      thr    MAP     RPM   dRPM"
            "   RPM/s  pred/s  blade  deg/s"
            "    HP   propHP   Qin    Qload   Qexcess"
        )
        print(
            " -----  -----  ------  ------  ------"
            "  ------  ------  ------  ------"
        ...
        for index, sample in enumerate(
            ...
            # 120 Hz simulation / 6 = 20 Hz trace.
            if index % 6 != 0:
                continue

            print(
                f" {sample['time']:5.2f}  "
                f"{sample['throttle']:5.3f}  "
                f"{sample['map']:6.2f}  "
                f"{sample['rpm']:6.1f}  "
                f"{sample['rpm_error']:+6.1f}  "
                f"{sample['rpm_dot']:+6.1f}  "
                f"{sample['rpm_dot_pred']:+6.1f}  "
                f"{sample['blade']:6.2f}  "
                f"{sample['blade_rate']:+6.1f}  "
                f"{sample['hp']:5.1f}  "
                f"{sample['prop_power_hp']:6.1f}  "
                f"{sample['engine_torque']:6.1f}  "
                f"{sample['prop_load_torque']:6.1f}  "
                f"{sample['excess_torque']:+7.1f}"
            )
