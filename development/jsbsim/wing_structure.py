...
# Documented M20M structural station landmarks visible in Mooney
# service-bulletin wing/spar figures.
#
# IMPORTANT:
# These are geometric/reference stations only.
# They are NOT claims that spar section properties change at every
# one of these locations.
#
# They must not be used to infer cap dimensions or EI values.
...
M20M_DOCUMENTED_MAIN_SPAR_ASSEMBLY_PN = "210000-517"


def wing_station_in_to_ft(
    wing_station_in: float,
) -> float:
    """
    Convert a Mooney wing station in inches from aircraft centerline
    into root-to-tip structural span coordinate in feet.
    """
    ...
    if not math.isfinite(
        ...
        raise ValueError(
            "Wing station must be finite"
        )

    if wing_station_in < 0.0:
        raise ValueError(
            "Wing station cannot be negative"
        )
...
@dataclass(frozen=True)
class SparGeometryAnchors:
    """
    Explicit spar-section geometry measurements at known span stations.

    These values are inputs, not estimates generated by the solver.

    Once controlled M20M drawings are available, their measured
    dimensions can populate this structure directly.
    """
...
def make_spar_geometry_anchors(
    *,
    y_ft: Sequence[float],
    cap_width_in: Sequence[float],
    cap_thickness_in: Sequence[float],
    ...
    """
    Validate and store measured spar-geometry anchor points.
    """
    ...
    if count < 2:
        raise ValueError(
            "At least two spar geometry anchors are required"
        )
    ...
    if any(
        ...
        raise ValueError(
            "Spar geometry anchor arrays must have equal length"
        )
    ...
    if not all(
        ...
        raise ValueError(
            "Spar geometry anchor positions must be finite"
        )
    ...
    for index in range(
        ...
        if (
            ...
            raise ValueError(
                "Spar geometry anchor positions must be strictly increasing"
            )
    ...
    for values in geometry_arrays:
        if not all(
            ...
            raise ValueError(
                "Spar geometry anchor values must be finite"
            )
        ...
        if any(
            ...
            raise ValueError(
                "Spar geometry anchor values must be positive"
            )

    return SparGeometryAnchors(
        y_ft=anchors_y,
        cap_width_in=tuple(
            float(value)
            for value in cap_width_in
...
def interpolate_positive_anchor_distribution(
    *,
    y_ft: Sequence[float],
    anchor_y_ft: Sequence[float],
    ...
    """
    Piecewise-linear interpolation for positive structural geometry.

    This performs interpolation only. It does not extrapolate beyond
    the measured anchor domain.
    """

    if len(anchor_y_ft) < 2:
        raise ValueError(
            "At least two geometry anchors are required"
        )
    ...
    if len(anchor_y_ft) != len(
        ...
        raise ValueError(
            "Geometry anchor arrays must have equal length"
        )
    ...
    if not all(
        ...
        raise ValueError(
            "Geometry anchor positions must be finite"
        )

    if not all(
        math.isfinite(value)
        for value in anchors_value
    ):
        raise ValueError(
            "Geometry anchor values must be finite"
        )
    ...
    if any(
        ...
        raise ValueError(
            "Geometry anchor values must be positive"
        )
    ...
    for index in range(
        ...
        if (
            ...
            raise ValueError(
                "Geometry anchor positions must be strictly increasing"
            )
    ...
    for raw_y in y_ft:
        ...
        if not math.isfinite(y):
            raise ValueError(
                "Output span station must be finite"
            )
        ...
        if (
            ...
            raise ValueError(
                "Output station lies outside geometry anchor domain"
            )
...
def make_spanwise_spar_sections_from_anchors(
    *,
    y_ft: Sequence[float],
    anchors: SparGeometryAnchors,
) -> SpanwiseSparSectionProperties:
    """
    Interpolate measured spar geometry onto the structural beam grid
    and calculate I(y).

    No extrapolation is permitted.
    """

    cap_width = interpolate_positive_anchor_distribution(
        y_ft=y_ft,
        anchor_y_ft=anchors.y_ft,
        anchor_values=anchors.cap_width_in,
    )

    cap_thickness = interpolate_positive_anchor_distribution(
        y_ft=y_ft,
        anchor_y_ft=anchors.y_ft,
        anchor_values=anchors.cap_thickness_in,
    )

    cap_separation = interpolate_positive_anchor_distribution(
        y_ft=y_ft,
        anchor_y_ft=anchors.y_ft,
        anchor_values=(
            anchors.cap_centroid_separation_in
        ),
    )

    web_thickness = interpolate_positive_anchor_distribution(
        y_ft=y_ft,
        anchor_y_ft=anchors.y_ft,
        anchor_values=anchors.web_thickness_in,
    )

    web_height = interpolate_positive_anchor_distribution(
        y_ft=y_ft,
        anchor_y_ft=anchors.y_ft,
        anchor_values=anchors.web_height_in,
    )

    return make_spanwise_spar_section_properties(
        y_ft=y_ft,
        cap_width_in=cap_width,
        cap_thickness_in=cap_thickness,
        cap_centroid_separation_in=cap_separation,
