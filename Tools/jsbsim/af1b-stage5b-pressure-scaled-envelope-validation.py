...
print(
    ...
    "PRESSURE-SCALED CAPABILITY VALIDATION"
)
print(
    "=============================================="
)
...
# Wider temporary dyno-load range than the original
# Stage-5A ISA harness so all temperature points can be kept
# away from the coarse-pitch stop.
CP_FACTORS = (
    1.25,
    1.40,
    1.55,
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    # Exact JSBSim ISA pressure at each test altitude.
    isa_pressure = {
            ...
            rows.append(
                {
                    ...
                    "pamb": current_pressure,
                    "isa_pamb": (
                        isa_pressure[
                            altitude_ft
                        ]
                    ),
                    "isa_cap": isa_capability,
                    "candidate_cap": (
                        candidate_capability
                    ),
                    "cp": best_factor,
...
print(
    " dT   Alt    Pamb   CapB   Density   CtrlTgt"
    "    MAP     HP     RPM   Blade    VE"
    "      Boost    Cap    Owner       Q"
)
print(
...
for row in rows:
    print(
        f"dT={row['temp_bias']:+3.0f}  "
        f"{row['altitude']/1000:4.0f}K  "
        f"MAP-Ctrl={row['map_error']:+.4f}\"  "
        f"Ctrl-Expected="
        f"{row['target_logic_error']:+.6f}\"  "
        f"RPMerr={row['rpm_error']:+.2f}"
    )


print()
print(
    "20K OWNERSHIP INSPECTION"
)
print(
    "------------------------"
)
...
for row in rows:
    ...
    print(
        ...
        f"CapB={row['candidate_cap']:.2f}\"  "
        f"Ctrl={s['controller_target']:.2f}\"  "
        f"MAP={s['map']:.2f}\"  "
        f"HP={s['hp']:.1f}  "
        f"Owner={row['owner']}"
    )
